...
time_step = seq(0.01,10,0.02) #extra fine scale
...
json_file <- "data/BDC_2022_all_direct_wFrames.json" #"data/BDC_2022_all_data.json"
...
current_event_all <- cbind(current_event_all, calc_vmag_ang(current_event_all)) # replace this with the next line if you commented out the relevant line in hockey_pipeline.R
# current_event_all <- current_event_all %>% calc_vmag_ang 
current_event <- current_event_all[t_track+1,]
current_event_all %>% head()
...
tracking_data = current_track %>% filter(frame_id==frame) %>% 
  mutate(goalie = ifelse(goalie=='TRUE',T,F))
...
loc_vel <- tracking_data %>% mutate(team_label = ifelse(team_name == offence,1,-1)) %>%
