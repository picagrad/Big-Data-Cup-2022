...
```
```{r}
point_val <- get_to_point(x_puck,y_puck,passes[162,],'Canada',want_plot=TRUE)
...
passes_with_vals = cbind(passes,values=all_point_val)
passes_with_vals #values are negative if USA controlled area, positive if Canada controlled area
```

```{r}
# Create image of this frame with potential passes

p <- ggplot(data = passes_with_vals,aes(x=x_coor, y=y_coor,z=values)) +
