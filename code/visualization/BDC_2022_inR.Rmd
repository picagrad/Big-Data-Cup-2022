...
```
```{r}
point_val = probs_to_point(x_puck,y_puck,new_pass[91,],'Canada',TRUE)
...
pass_potential <- cbind(new_pass,probs=all_point_val)
pass_potential$Probability = (pass_potential$probs-min(pass_potential$probs))/(max(pass_potential$probs)-min(pass_potential$probs))
pass_potential
...
p <- ggplot(data = pass_potential,aes(x = x, y = y)) + 
    geom_point(aes(color = probs),size=2, shape=16) +
