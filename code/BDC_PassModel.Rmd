---
title: "BDC_PassModel"
output: pdf_document
---

```{r}
load.libraries = c("rjson", "jsonlite", "tidyverse", "gganimate","ggpmisc","ggnewscale","viridis","tictoc","scales")
install.lib = load.libraries[!load.libraries %in% installed.packages()]
for (libs in install.lib) {install.packages(libs, dependencies = TRUE)}
sapply(load.libraries, require, character = TRUE)
```

```{r}
tic()
time_step = seq(0.01,10,0.05) #extra fine scale
#time_step = seq(0.05,10,0.05) #regular scale
#theta_scale = 0.01 #extra fine scale
theta_scale = 0.05 #regular scale
#speed_puck = 90 #ft/s
speed_puck = 55 #ft/s
#speed_puck = 40 #ft/s

source("C:/Users/Paula/Desktop/Big-Data-Cup-2022-Private/code/visualization/hockey_pipeline.R")
setwd("C:/Users/Paula/Desktop/Big-Data-Cup-2022-Private")
json_file <- "data/BDC_2022_passOnly.json"
dat <- fromJSON(json_file)

# Set up the event data to be in a data frame. 
json_events <- lapply(dat[1:28], function(x) {
  x[sapply(x, is.null)] <- NA
  unlist(x)
})
current_event <- as.data.frame(do.call("cbind", json_events))
current_event$team_name[current_event$team_name=="Olympic (Women) - Canada"]="Canada"
current_event$team_name[current_event$team_name=="Olympic (Women) - United States"]="USA"
current_event$team_name[current_event$team_name=="Olympic (Women) - Finland"]="Finland"
current_event$team_name[current_event$team_name=="Olympic (Women) - Olympic Athletes from Russia"]="ROC"
current_event$team_name[current_event$team_name=="Olympic (Women) - Switzerland"]="Switzerland"

current_event$x_coord = current_event$x_coord %>% as.double()
current_event$y_coord = current_event$y_coord %>% as.double()
current_event$x_coord_2 = current_event$x_coord_2 %>% as.double()
current_event$y_coord_2 = current_event$y_coord_2 %>% as.double()

current_event$x_coord[current_event$x_coord < 100] = 200-current_event$x_coord[current_event$x_coord < 100]
current_event$x_coord_2[which(current_event$x_coord_2 < 100)] = 200-current_event$x_coord_2[which(current_event$x_coord_2 < 100)]

current_event$y_coord = 85-current_event$y_coord
current_event$y_coord_2 = 85-current_event$y_coord_2

vel_ang = calc_vmag_ang(current_event)
current_event$vel_init = vel_ang[,1]
current_event$ang_init = vel_ang[,2]
toc()
```

```{r}
tic()
pass_speed = vector(mode = "list", length = 5)
n_good=0
index_spot=1
all_speed = c(45,60,75,90)
for(speed in 1:5){
  pass_score = vector(mode = "list", length = nrow(current_event))
  for(t_track in 1:nrow(current_event)){
    # Set up the tracking data
    line1 <- paste('dat$tracks$\'',(t_track-1),'\'',sep='')
    line2 <- eval(parse(text=line1))
    if(length(line2$frame_id)!=0){
      json_tracks <- lapply(line2, function(x) {
        x[sapply(x, is.null)] <- NA
        unlist(x)
      })
      current_track <- as.data.frame(do.call("cbind", json_tracks))
    }
    current_track$frame_id = current_track$frame_id %>% as.integer()
    current_track$period = current_track$period %>% as.integer()
    current_track$track_id = current_track$track_id %>% as.integer()
    current_track$jersey_number = current_track$jersey_number %>% as.integer()
    current_track$x_ft = current_track$x_ft %>% as.double()
    current_track$y_ft = current_track$y_ft %>% as.double()
    current_track$vel_x = current_track$vel_x %>% as.double()
    current_track$vel_y = current_track$vel_y %>% as.double()
    
    current_track$vel_x[which(current_track$vel_x==0)]=0.05
    current_track$vel_y[which(current_track$vel_y==0)]=0.05
    current_track$goalie = ifelse(current_track$goalie =='TRUE',T,F)
    
    if(current_track$x_ft[1]<100){
      current_track$x_ft = 200 - current_track$x_ft
      current_track$vel_x = -current_track$vel_x
    }
    
    if(speed==5){
      # determine speed of actual puck 
      speed_puck = current_event$vel_init[t_track]
      if(is.na(speed_puck)){
        speed_puck=50 #current_event$vel_init %>% mean(na.rm=TRUE) gives 46.0478, as we already did 45, we will default to 50
      }
    }else{
      speed_puck = all_speed[speed]
    }
    
    event_info <- current_event[t_track,] %>% select(team_name,Player_1_num,Player_2_num)
    off <- event_info$team_name
    puck = current_track[current_track$jersey_number==event_info$Player_1_num,] %>% select(team_name,x_ft,y_ft)
    puck = puck[puck$team_name==off,]
    
    n_players <- table(current_track$team_name)
    n_off <- sum(n_players[names(n_players)==off])
    n_def <- sum(n_players[names(n_players)!=off])
    if(nrow(puck)>0 & length(n_players)==2 & n_off>2 & n_def>2){
      
      #x_p = puck$x_ft+2
      #y_p = puck$y_ft-0.5
      x_p = current_event[t_track,"x_coord"]#puck$x_ft
      y_p = current_event[t_track,"y_coord"]#puck$y_ft
      if(abs(y_p-puck$y_ft)>20){
        y_p = 85-y_p
      }
      source("C:/Users/Paula/Desktop/Big-Data-Cup-2022-Private/code/visualization/hockey_pipeline.R")

      
      theta = seq(-pi,pi,by=theta_scale)
      passes = data.frame(angle=c(),x=c(),y=c(), t=c())
      for(angle in theta){
        passes = rbind(passes,cbind(angle,puck_motion_model2(x_p,y_p,angle)))
      }
      new_pass <- clean_pass(passes)
      calc_pass <- new_pass %>% group_by(angle) %>% top_n(1, t)
      
      new_pass <- rbind(new_pass,c(new_pass$angle[1],x_p,y_p,0))
      
      xyt <- new_pass %>% select(x,y,t)
      loc_vel <- current_track %>% mutate(team_label = ifelse(team_name == off,1,-1)) %>%
        select(x_ft,y_ft,vel_x,vel_y, team_label, goalie) 
      # options(warn=-1)
      new_pass <- new_pass %>% 
        mutate(all_ctrl = teamwise_ice_ctrl_xyt(loc_vel, xyt), score_prob = apply(xyt,1,score_prob)) %>%
        mutate(pass_value = score_prob * ((all_ctrl+1)/2)^1)
      # options(warn=0)
      
      all_point_val <- apply(calc_pass,1,probs_to_point,x_puck=x_p,y_puck=y_p,all_ang=new_pass,tracks1=current_track,offence=off,want_plot=FALSE)
      pass_potential <- bind_rows(all_point_val, .id = "column_label")
      
      pass_score[[t_track]]<- pass_potential
      print(t_track)
      n_good=n_good+1
    }else{
      print(paste("Pass track failed conditions:",t_track))
    }
    
  }
  pass_speed[[index_spot]]=pass_score
  index_spot = index_spot+1
}
save(pass_speed, file="C:/Users/Paula/Desktop/Big-Data-Cup-2022-Private/data/pass_model_5speeds.Rdata")
toc()
```

```{r}
#perform quantile normalization
combine_dat <-cbind("speed"=1,"id"=2,pass_speed[[1]][[2]])
for(j in 1:5){
  if(j!=1){
    combine_dat <- rbind(combine_dat, cbind("speed"=j,"id"=2,pass_speed[[j]][[2]]))
  }
  for(i in 3:length(pass_speed[[j]])){
    if(length(pass_speed[[j]][[i]])>0){
      combine_dat <- rbind(combine_dat, cbind("speed"=j,"id"=i,pass_speed[[j]][[i]]))
    }
  }
  print(j)
}
save(combine_dat, file="C:/Users/Paula/Desktop/Big-Data-Cup-2022-Private/data/combine_data.Rdata")

#to adjust c("adj_pass_value","location_pass_value", "expected_pass_value")
the_cols = c(11,15,17)
all_rank = apply(combine_dat[,the_cols],2,rank)
quant_rank1 = all_rank/nrow(combine_dat)

quant_rank = cbind(combine_dat[,c(1,2)],quant_rank1)
save(quant_rank, file="C:/Users/Paula/Desktop/Big-Data-Cup-2022-Private/data/quant_columns.Rdata")

pass_speed2 <- pass_speed
for(j in 1:5){
  for(i in 1:length(pass_speed[[j]])){
    if(length(pass_speed[[j]][[i]])>0){
      pass_speed2[[j]][[i]][,the_cols] = (quant_rank %>% filter(speed==j) %>% filter(id==i))[,3:5]
    }
  }
  print(j)
}
save(pass_speed2, file="C:/Users/Paula/Desktop/Big-Data-Cup-2022-Private/data/pass_model_5speeds_quantile.Rdata")
```

```{r}
current_event$max_best_case_within_45 = NaN
current_event$ang_best_case_overall_45 = NaN
current_event$max_best_case_overall_45 = NaN
current_event$max_best_case_within_60 = NaN
current_event$ang_best_case_overall_60 = NaN
current_event$max_best_case_overall_60 = NaN
current_event$max_best_case_within_75 = NaN
current_event$ang_best_case_overall_75 = NaN
current_event$max_best_case_overall_75 = NaN
current_event$max_best_case_within_90 = NaN
current_event$ang_best_case_overall_90 = NaN
current_event$max_best_case_overall_90 = NaN
current_event$max_best_case_within_Vel_init = NaN
current_event$ang_best_case_overall_Vel_init = NaN
current_event$max_best_case_overall_Vel_init = NaN

current_event$max_keep_possession_within_45 = NaN
current_event$ang_keep_possession_overall_45 = NaN
current_event$max_keep_possession_overall_45 = NaN
current_event$max_keep_possession_within_60 = NaN
current_event$ang_keep_possession_overall_60 = NaN
current_event$max_keep_possession_overall_60 = NaN
current_event$max_keep_possession_within_75 = NaN
current_event$ang_keep_possession_overall_75 = NaN
current_event$max_keep_possession_overall_75 = NaN
current_event$max_keep_possession_within_90 = NaN
current_event$ang_keep_possession_overall_90 = NaN
current_event$max_keep_possession_overall_90 = NaN
current_event$max_keep_possession_within_Vel_init = NaN
current_event$ang_keep_possession_overall_Vel_init = NaN
current_event$max_keep_possession_overall_Vel_init = NaN

current_event$max_expected_pass_value_within_45 = NaN
current_event$ang_expected_pass_value_overall_45 = NaN
current_event$max_expected_pass_value_overall_45 = NaN
current_event$max_expected_pass_value_within_60 = NaN
current_event$ang_expected_pass_value_overall_60 = NaN
current_event$max_expected_pass_value_overall_60 = NaN
current_event$max_expected_pass_value_within_75 = NaN
current_event$ang_expected_pass_value_overall_75 = NaN
current_event$max_expected_pass_value_overall_75 = NaN
current_event$max_expected_pass_value_within_90 = NaN
current_event$ang_expected_pass_value_overall_90 = NaN
current_event$max_expected_pass_value_overall_90 = NaN
current_event$max_expected_pass_value_within_Vel_init = NaN
current_event$ang_expected_pass_value_overall_Vel_init = NaN
current_event$max_expected_pass_value_overall_Vel_init = NaN

current_event$passer_pass_value = NaN
```


```{r}
#0.05*10*180/pi gives us a range of about 30 degrees around the intended target

for(track_pick in 1:nrow(current_event)){
  ang_shot = current_event$ang_init[track_pick]
  
  #speed 45
  track_pass = pass_speed2[[1]][[track_pick]]
  
  if(!is.null(track_pass)){
    current_event$passer_pass_value[track_pick] = track_pass[which(track_pass$t==0),"pass_value"]
    target_ang = track_pass[which.min(abs(track_pass$theta-ang_shot)),]
    all_within_ang = track_pass[!is.na(match(round(track_pass$theta,4),round(target_ang$theta+seq(-0.25,0.25,by=theta_scale),4))),]
    current_event$max_best_case_within_45[track_pick] = max(all_within_ang$best_case_pass_value)
    current_event$ang_best_case_overall_45[track_pick] = track_pass[which.max(track_pass$best_case_pass_value),"theta"]
    current_event$max_best_case_overall_45[track_pick] = track_pass[which.max(track_pass$best_case_pass_value),"best_case_pass_value"]
    current_event$max_keep_possession_within_45[track_pick] = max(all_within_ang$keep_possesion_prob)
    current_event$ang_keep_possession_overall_45[track_pick] = track_pass[which.max(track_pass$keep_possesion_prob),"theta"]
    current_event$max_keep_possession_overall_45[track_pick] = track_pass[which.max(track_pass$keep_possesion_prob),"keep_possesion_prob"]
    current_event$max_expected_pass_value_within_45[track_pick] = max(all_within_ang$expected_pass_value)
    current_event$ang_expected_pass_value_overall_45[track_pick] = track_pass[which.max(track_pass$expected_pass_value),"theta"]
    current_event$max_expected_pass_value_overall_45[track_pick] =track_pass[which.max(track_pass$expected_pass_value),"expected_pass_value"]
    
    #speed 60
    track_pass = pass_speed2[[2]][[track_pick]]
    target_ang = track_pass[which.min(abs(track_pass$theta-ang_shot)),]
    all_within_ang = track_pass[!is.na(match(round(track_pass$theta,4),round(target_ang$theta+seq(-0.25,0.25,by=theta_scale),4))),]
    current_event$max_best_case_within_60[track_pick] = max(all_within_ang$best_case_pass_value)
    current_event$ang_best_case_overall_60[track_pick] = track_pass[which.max(track_pass$best_case_pass_value),"theta"]
    current_event$max_best_case_overall_60[track_pick] =track_pass[which.max(track_pass$best_case_pass_value),"best_case_pass_value"]
    current_event$max_keep_possession_within_60[track_pick] = max(all_within_ang$keep_possesion_prob)
    current_event$ang_keep_possession_overall_60[track_pick] = track_pass[which.max(track_pass$keep_possesion_prob),"theta"]
    current_event$max_keep_possession_overall_60[track_pick] = track_pass[which.max(track_pass$keep_possesion_prob),"keep_possesion_prob"]
    current_event$max_expected_pass_value_within_60[track_pick] = max(all_within_ang$expected_pass_value)
    current_event$ang_expected_pass_value_overall_60[track_pick] = track_pass[which.max(track_pass$expected_pass_value),"theta"]
    current_event$max_expected_pass_value_overall_60[track_pick] = track_pass[which.max(track_pass$expected_pass_value),"expected_pass_value"]
    
    #speed 75
    track_pass = pass_speed2[[3]][[track_pick]]
    target_ang = track_pass[which.min(abs(track_pass$theta-ang_shot)),]
    all_within_ang = track_pass[!is.na(match(round(track_pass$theta,4),round(target_ang$theta+seq(-0.25,0.25,by=theta_scale),4))),]
    current_event$max_best_case_within_75[track_pick] = max(all_within_ang$best_case_pass_value)
    current_event$ang_best_case_overall_75[track_pick] = track_pass[which.max(track_pass$best_case_pass_value),"theta"]
    current_event$max_best_case_overall_75[track_pick] = track_pass[which.max(track_pass$best_case_pass_value),"best_case_pass_value"]
    current_event$max_keep_possession_within_75[track_pick] = max(all_within_ang$keep_possesion_prob)
    current_event$ang_keep_possession_overall_75[track_pick] = track_pass[which.max(track_pass$keep_possesion_prob),"theta"]
    current_event$max_keep_possession_overall_75[track_pick] = track_pass[which.max(track_pass$keep_possesion_prob),"keep_possesion_prob"]
    current_event$max_expected_pass_value_within_75[track_pick] = max(all_within_ang$expected_pass_value)
    current_event$ang_expected_pass_value_overall_75[track_pick] = track_pass[which.max(track_pass$expected_pass_value),"theta"]
    current_event$max_expected_pass_value_overall_75[track_pick] = track_pass[which.max(track_pass$expected_pass_value),"expected_pass_value"]
    
    #speed 90
    track_pass = pass_speed2[[4]][[track_pick]]
    target_ang = track_pass[which.min(abs(track_pass$theta-ang_shot)),]
    all_within_ang = track_pass[!is.na(match(round(track_pass$theta,4),round(target_ang$theta+seq(-0.25,0.25,by=theta_scale),4))),]
    current_event$max_best_case_within_90[track_pick] = max(all_within_ang$best_case_pass_value)
    current_event$ang_best_case_overall_90[track_pick] = track_pass[which.max(track_pass$best_case_pass_value),"theta"]
    current_event$max_best_case_overall_90[track_pick] = track_pass[which.max(track_pass$best_case_pass_value),"best_case_pass_value"]
    current_event$max_keep_possession_within_90[track_pick] = max(all_within_ang$keep_possesion_prob)
    current_event$ang_keep_possession_overall_90[track_pick] = track_pass[which.max(track_pass$keep_possesion_prob),"theta"]
    current_event$max_keep_possession_overall_90[track_pick] = track_pass[which.max(track_pass$keep_possesion_prob),"keep_possesion_prob"]
    current_event$max_expected_pass_value_within_90[track_pick] = max(all_within_ang$expected_pass_value)
    current_event$ang_expected_pass_value_overall_90[track_pick] = track_pass[which.max(track_pass$expected_pass_value),"theta"]
    current_event$max_expected_pass_value_overall_90[track_pick] =track_pass[which.max(track_pass$expected_pass_value),"expected_pass_value"]
    
    #speed Vel_init
    track_pass = pass_speed2[[5]][[track_pick]]
    target_ang = track_pass[which.min(abs(track_pass$theta-ang_shot)),]
    all_within_ang = track_pass[!is.na(match(round(track_pass$theta,4),round(target_ang$theta+seq(-0.25,0.25,by=theta_scale),4))),]
    current_event$max_best_case_within_Vel_init[track_pick] = max(all_within_ang$best_case_pass_value)
    current_event$ang_best_case_overall_Vel_init[track_pick] = track_pass[which.max(track_pass$best_case_pass_value),"theta"]
    current_event$max_best_case_overall_Vel_init[track_pick] = track_pass[which.max(track_pass$best_case_pass_value),"best_case_pass_value"] 
    current_event$max_keep_possession_within_Vel_init[track_pick] = max(all_within_ang$keep_possesion_prob)
    current_event$ang_keep_possession_overall_Vel_init[track_pick] = track_pass[which.max(track_pass$keep_possesion_prob),"theta"]
    current_event$max_keep_possession_overall_Vel_init[track_pick] = track_pass[which.max(track_pass$keep_possesion_prob),"keep_possesion_prob"]
    current_event$max_expected_pass_value_within_Vel_init[track_pick] = max(all_within_ang$expected_pass_value)
    current_event$ang_expected_pass_value_overall_Vel_init[track_pick] = track_pass[which.max(track_pass$expected_pass_value),"theta"]
    current_event$max_expected_pass_value_overall_Vel_init[track_pick] =track_pass[which.max(track_pass$expected_pass_value),"expected_pass_value"]
  }
}

```

```{r}
save(current_event, file="C:/Users/Paula/Desktop/Big-Data-Cup-2022-Private/data/pass_summary_quantile.Rdata")
```

```{r}
load("C:/Users/Paula/Desktop/Big-Data-Cup-2022-Private/data/pass_model_5speeds.Rdata")
load("C:/Users/Paula/Desktop/Big-Data-Cup-2022-Private/data/pass_summary.Rdata")
```

```{r}
plot(current_event$max_expected_pass_value_within_Vel_init,current_event$max_keep_possession_within_Vel_init)
plot(current_event$max_best_case_within_Vel_init,current_event$max_keep_possession_within_Vel_init)
plot(current_event$max_expected_pass_value_within_Vel_init,current_event$max_best_case_within_Vel_init)
```


```{r}
track_pick = 11
t_pick = 5
# Set up the tracking data
line1 <- paste('dat$tracks$\'',(track_pick-1),'\'',sep='')
line2 <- eval(parse(text=line1))
if(length(line2$frame_id)!=0){
  json_tracks <- lapply(line2, function(x) {
    x[sapply(x, is.null)] <- NA
    unlist(x)
  })
  current_track <- as.data.frame(do.call("cbind", json_tracks))
}
current_track$frame_id = current_track$frame_id %>% as.integer()
current_track$period = current_track$period %>% as.integer()
current_track$track_id = current_track$track_id %>% as.integer()
current_track$jersey_number = current_track$jersey_number %>% as.integer()
current_track$x_ft = current_track$x_ft %>% as.double()
current_track$y_ft = current_track$y_ft %>% as.double()
current_track$vel_x = current_track$vel_x %>% as.double()
current_track$vel_y = current_track$vel_y %>% as.double()

current_track$vel_x[which(current_track$vel_x==0)]=0.05
current_track$vel_y[which(current_track$vel_y==0)]=0.05
current_track$goalie = ifelse(current_track$goalie =='TRUE',T,F)

if(current_track$x_ft[1]<100){
  current_track$x_ft = 200 - current_track$x_ft
  current_track$vel_x = -current_track$vel_x
}

event_info <- current_event[track_pick,] %>% select(team_name,Player_1_num,Player_2_num)
off <- event_info$team_name
puck = current_track[current_track$jersey_number==event_info$Player_1_num,] %>% select(team_name,x_ft,y_ft)
puck = puck[puck$team_name==off,]

x_p = puck$x_ft+2
y_p = puck$y_ft-0.5
    
p <- ggplot(data = pass_speed2[[t_pick]][[track_pick]],aes(x = x, y = y)) + 
    geom_point(aes(color = all_ctrl),size=2, shape=16) +
    scale_colour_gradient2(na.value="white",low = muted("green"), mid = "white", high = muted("purple"))+#,midpoint = 0.5
    scale_x_continuous(expand = c(0, 0)) +
    scale_y_continuous(expand = c(0, 0)) +
  #geom_point(aes(x = x, y = y), size = 1, shape = 4)+
  new_scale_fill() +
  new_scale_color() +
  geom_point(data=current_track,aes(x = x_ft, y = y_ft, fill = team_name), size = 5, shape = 21) +
  geom_text(data=current_track,aes(x = x_ft, y = y_ft, label = jersey_number, colour = team_name), size = 3) +
  geom_point(aes(x = x_p, y = y_p), size = 2, shape = 16, colour='black') +
  scale_colour_manual(values = c("USA" = "white", "Canada" = "white")) +
  scale_fill_manual(values = c("USA" = "blue", "Canada" = "red")) +
  labs(fill = "Team") +
  guides(colour = "none")+ 
  geom_segment(data=current_track,aes(x = x_ft, y = y_ft, xend = x_ft+vel_x, yend = y_ft+vel_y), #/sqrt(vel_x^2+vel_y^2) to get r=1
  arrow = arrow(length = unit(0.2, "cm")),size=1, colour='cyan')

p2 <- ggplot(data = pass_speed2[[t_pick]][[track_pick]],aes(x = x, y = y)) + 
    geom_point(aes(color = score_prob),size=2, shape=16) +
    scale_colour_gradient2(na.value="white",low = muted("green"), mid = "white", high = muted("purple"))+#,midpoint = 0.5
    scale_x_continuous(expand = c(0, 0)) +
    scale_y_continuous(expand = c(0, 0)) +
  #geom_point(aes(x = x, y = y), size = 1, shape = 4)+
  new_scale_fill() +
  new_scale_color() +
  geom_point(data=current_track,aes(x = x_ft, y = y_ft, fill = team_name), size = 5, shape = 21) +
  geom_text(data=current_track,aes(x = x_ft, y = y_ft, label = jersey_number, colour = team_name), size = 3) +
  geom_point(aes(x = x_p, y = y_p), size = 2, shape = 16, colour='black') +
  scale_colour_manual(values = c("USA" = "white", "Canada" = "white")) +
  scale_fill_manual(values = c("USA" = "blue", "Canada" = "red")) +
  labs(fill = "Team") +
  guides(colour = "none")+ 
  geom_segment(data=current_track,aes(x = x_ft, y = y_ft, xend = x_ft+vel_x, yend = y_ft+vel_y), #/sqrt(vel_x^2+vel_y^2) to get r=1
  arrow = arrow(length = unit(0.2, "cm")),size=1, colour='cyan')

p3 <- ggplot(data = pass_speed2[[t_pick]][[track_pick]],aes(x = x, y = y)) + 
    geom_point(aes(color = pass_value),size=2, shape=16) +
    scale_colour_gradient2(na.value="white",low = muted("green"), mid = "white", high = muted("purple"))+#,midpoint = 0.5
    scale_x_continuous(expand = c(0, 0)) +
    scale_y_continuous(expand = c(0, 0)) +
  #geom_point(aes(x = x, y = y), size = 1, shape = 4)+
  new_scale_fill() +
  new_scale_color() +
  geom_point(data=current_track,aes(x = x_ft, y = y_ft, fill = team_name), size = 5, shape = 21) +
  geom_text(data=current_track,aes(x = x_ft, y = y_ft, label = jersey_number, colour = team_name), size = 3) +
  geom_point(aes(x = x_p, y = y_p), size = 2, shape = 16, colour='black') +
  scale_colour_manual(values = c("USA" = "white", "Canada" = "white")) +
  scale_fill_manual(values = c("USA" = "blue", "Canada" = "red")) +
  labs(fill = "Team") +
  guides(colour = "none")+ 
  geom_segment(data=current_track,aes(x = x_ft, y = y_ft, xend = x_ft+vel_x, yend = y_ft+vel_y), #/sqrt(vel_x^2+vel_y^2) to get r=1
  arrow = arrow(length = unit(0.2, "cm")),size=1, colour='cyan')

p4 <- ggplot(data = pass_speed2[[t_pick]][[track_pick]],aes(x = x, y = y)) + 
    geom_point(aes(color = adj_pass_value),size=2, shape=16) +
    scale_colour_gradient2(na.value="white",low = muted("green"), mid = "white", high = muted("purple"))+#,midpoint = 0.5
    scale_x_continuous(expand = c(0, 0)) +
    scale_y_continuous(expand = c(0, 0)) +
  #geom_point(aes(x = x, y = y), size = 1, shape = 4)+
  new_scale_fill() +
  new_scale_color() +
  geom_point(data=current_track,aes(x = x_ft, y = y_ft, fill = team_name), size = 5, shape = 21) +
  geom_text(data=current_track,aes(x = x_ft, y = y_ft, label = jersey_number, colour = team_name), size = 3) +
  geom_point(aes(x = x_p, y = y_p), size = 2, shape = 16, colour='black') +
  scale_colour_manual(values = c("USA" = "white", "Canada" = "white")) +
  scale_fill_manual(values = c("USA" = "blue", "Canada" = "red")) +
  labs(fill = "Team") +
  guides(colour = "none")+ 
  geom_segment(data=current_track,aes(x = x_ft, y = y_ft, xend = x_ft+vel_x, yend = y_ft+vel_y), #/sqrt(vel_x^2+vel_y^2) to get r=1
  arrow = arrow(length = unit(0.2, "cm")),size=1, colour='cyan')
par(mfrow = c(2, 2))

p5 <- ggplot(data = pass_speed2[[t_pick]][[track_pick]],aes(x = x, y = y)) + 
    geom_point(aes(color = off_prob),size=2, shape=16) +
    scale_colour_gradient2(na.value="white",low = muted("green"), mid = "white", high = muted("purple"))+#,midpoint = 0.5
    scale_x_continuous(expand = c(0, 0)) +
    scale_y_continuous(expand = c(0, 0)) +
  #geom_point(aes(x = x, y = y), size = 1, shape = 4)+
  new_scale_fill() +
  new_scale_color() +
  geom_point(data=current_track,aes(x = x_ft, y = y_ft, fill = team_name), size = 5, shape = 21) +
  geom_text(data=current_track,aes(x = x_ft, y = y_ft, label = jersey_number, colour = team_name), size = 3) +
  geom_point(aes(x = x_p, y = y_p), size = 2, shape = 16, colour='black') +
  scale_colour_manual(values = c("USA" = "white", "Canada" = "white")) +
  scale_fill_manual(values = c("USA" = "blue", "Canada" = "red")) +
  labs(fill = "Team") +
  guides(colour = "none")+ 
  geom_segment(data=current_track,aes(x = x_ft, y = y_ft, xend = x_ft+vel_x, yend = y_ft+vel_y), #/sqrt(vel_x^2+vel_y^2) to get r=1
  arrow = arrow(length = unit(0.2, "cm")),size=1, colour='cyan')
par(mfrow = c(2, 2))

p6 <- ggplot(data = pass_speed2[[t_pick]][[track_pick]],aes(x = x, y = y)) + 
    geom_point(aes(color = location_pass_value),size=2, shape=16) +
    scale_colour_gradient2(na.value="white",low = muted("green"), mid = "white", high = muted("purple"))+#,midpoint = 0.5
    scale_x_continuous(expand = c(0, 0)) +
    scale_y_continuous(expand = c(0, 0)) +
  #geom_point(aes(x = x, y = y), size = 1, shape = 4)+
  new_scale_fill() +
  new_scale_color() +
  geom_point(data=current_track,aes(x = x_ft, y = y_ft, fill = team_name), size = 5, shape = 21) +
  geom_text(data=current_track,aes(x = x_ft, y = y_ft, label = jersey_number, colour = team_name), size = 3) +
  geom_point(aes(x = x_p, y = y_p), size = 2, shape = 16, colour='black') +
  scale_colour_manual(values = c("USA" = "white", "Canada" = "white")) +
  scale_fill_manual(values = c("USA" = "blue", "Canada" = "red")) +
  labs(fill = "Team") +
  guides(colour = "none")+ 
  geom_segment(data=current_track,aes(x = x_ft, y = y_ft, xend = x_ft+vel_x, yend = y_ft+vel_y), #/sqrt(vel_x^2+vel_y^2) to get r=1
  arrow = arrow(length = unit(0.2, "cm")),size=1, colour='cyan')
par(mfrow = c(2, 2))

p7 <- ggplot(data = pass_speed2[[t_pick]][[track_pick]],aes(x = x, y = y)) + 
    geom_point(aes(color = keep_possesion_prob),size=2, shape=16) +
    scale_colour_gradient2(na.value="white",low = muted("green"), mid = "white", high = muted("purple"))+#,midpoint = 0.5
    scale_x_continuous(expand = c(0, 0)) +
    scale_y_continuous(expand = c(0, 0)) +
  #geom_point(aes(x = x, y = y), size = 1, shape = 4)+
  new_scale_fill() +
  new_scale_color() +
  geom_point(data=current_track,aes(x = x_ft, y = y_ft, fill = team_name), size = 5, shape = 21) +
  geom_text(data=current_track,aes(x = x_ft, y = y_ft, label = jersey_number, colour = team_name), size = 3) +
  geom_point(aes(x = x_p, y = y_p), size = 2, shape = 16, colour='black') +
  scale_colour_manual(values = c("USA" = "white", "Canada" = "white")) +
  scale_fill_manual(values = c("USA" = "blue", "Canada" = "red")) +
  labs(fill = "Team") +
  guides(colour = "none")+ 
  geom_segment(data=current_track,aes(x = x_ft, y = y_ft, xend = x_ft+vel_x, yend = y_ft+vel_y), #/sqrt(vel_x^2+vel_y^2) to get r=1
  arrow = arrow(length = unit(0.2, "cm")),size=1, colour='cyan')
par(mfrow = c(2, 2))

p8 <- ggplot(data = pass_speed2[[t_pick]][[track_pick]],aes(x = x, y = y)) + 
    geom_point(aes(color = expected_pass_value),size=2, shape=16) +
    scale_colour_gradient2(na.value="white",low = muted("green"), mid = "white", high = muted("purple"))+#,midpoint = 0.5
    scale_x_continuous(expand = c(0, 0)) +
    scale_y_continuous(expand = c(0, 0)) +
  #geom_point(aes(x = x, y = y), size = 1, shape = 4)+
  new_scale_fill() +
  new_scale_color() +
  geom_point(data=current_track,aes(x = x_ft, y = y_ft, fill = team_name), size = 5, shape = 21) +
  geom_text(data=current_track,aes(x = x_ft, y = y_ft, label = jersey_number, colour = team_name), size = 3) +
  geom_point(aes(x = x_p, y = y_p), size = 2, shape = 16, colour='black') +
  scale_colour_manual(values = c("USA" = "white", "Canada" = "white")) +
  scale_fill_manual(values = c("USA" = "blue", "Canada" = "red")) +
  labs(fill = "Team") +
  guides(colour = "none")+ 
  geom_segment(data=current_track,aes(x = x_ft, y = y_ft, xend = x_ft+vel_x, yend = y_ft+vel_y), #/sqrt(vel_x^2+vel_y^2) to get r=1
  arrow = arrow(length = unit(0.2, "cm")),size=1, colour='cyan')
par(mfrow = c(2, 2))

p9 <- ggplot(data = pass_speed2[[t_pick]][[track_pick]],aes(x = x, y = y)) + 
    geom_point(aes(color = max_pass_value),size=2, shape=16) +
    scale_colour_gradient2(na.value="white",low = muted("green"), mid = "white", high = muted("purple"))+#,midpoint = 0.5
    scale_x_continuous(expand = c(0, 0)) +
    scale_y_continuous(expand = c(0, 0)) +
  #geom_point(aes(x = x, y = y), size = 1, shape = 4)+
  new_scale_fill() +
  new_scale_color() +
  geom_point(data=current_track,aes(x = x_ft, y = y_ft, fill = team_name), size = 5, shape = 21) +
  geom_text(data=current_track,aes(x = x_ft, y = y_ft, label = jersey_number, colour = team_name), size = 3) +
  geom_point(aes(x = x_p, y = y_p), size = 2, shape = 16, colour='black') +
  scale_colour_manual(values = c("USA" = "white", "Canada" = "white")) +
  scale_fill_manual(values = c("USA" = "blue", "Canada" = "red")) +
  labs(fill = "Team") +
  guides(colour = "none")+ 
  geom_segment(data=current_track,aes(x = x_ft, y = y_ft, xend = x_ft+vel_x, yend = y_ft+vel_y), #/sqrt(vel_x^2+vel_y^2) to get r=1
  arrow = arrow(length = unit(0.2, "cm")),size=1, colour='cyan')
par(mfrow = c(2, 2))

p10 <- ggplot(data = pass_speed2[[t_pick]][[track_pick]],aes(x = x, y = y)) + 
    geom_point(aes(color = best_case_pass_value),size=2, shape=16) +
    scale_colour_gradient2(na.value="white",low = muted("green"), mid = "white", high = muted("purple"))+#,midpoint = 0.5
    scale_x_continuous(expand = c(0, 0)) +
    scale_y_continuous(expand = c(0, 0)) +
  #geom_point(aes(x = x, y = y), size = 1, shape = 4)+
  new_scale_fill() +
  new_scale_color() +
  geom_point(data=current_track,aes(x = x_ft, y = y_ft, fill = team_name), size = 5, shape = 21) +
  geom_text(data=current_track,aes(x = x_ft, y = y_ft, label = jersey_number, colour = team_name), size = 3) +
  geom_point(aes(x = x_p, y = y_p), size = 2, shape = 16, colour='black') +
  scale_colour_manual(values = c("USA" = "white", "Canada" = "white")) +
  scale_fill_manual(values = c("USA" = "blue", "Canada" = "red")) +
  labs(fill = "Team") +
  guides(colour = "none")+ 
  geom_segment(data=current_track,aes(x = x_ft, y = y_ft, xend = x_ft+vel_x, yend = y_ft+vel_y), #/sqrt(vel_x^2+vel_y^2) to get r=1
  arrow = arrow(length = unit(0.2, "cm")),size=1, colour='cyan')
par(mfrow = c(2, 2))

p11 <- ggplot(data = pass_speed2[[t_pick]][[track_pick]],aes(x = x, y = y)) + 
    geom_point(aes(color = successful_pass_prob),size=2, shape=16) +
    scale_colour_gradient2(na.value="white",low = muted("green"), mid = "white", high = muted("purple"))+#,midpoint = 0.5
    scale_x_continuous(expand = c(0, 0)) +
    scale_y_continuous(expand = c(0, 0)) +
  #geom_point(aes(x = x, y = y), size = 1, shape = 4)+
  new_scale_fill() +
  new_scale_color() +
  geom_point(data=current_track,aes(x = x_ft, y = y_ft, fill = team_name), size = 5, shape = 21) +
  geom_text(data=current_track,aes(x = x_ft, y = y_ft, label = jersey_number, colour = team_name), size = 3) +
  geom_point(aes(x = x_p, y = y_p), size = 2, shape = 16, colour='black') +
  scale_colour_manual(values = c("USA" = "white", "Canada" = "white")) +
  scale_fill_manual(values = c("USA" = "blue", "Canada" = "red")) +
  labs(fill = "Team") +
  guides(colour = "none")+ 
  geom_segment(data=current_track,aes(x = x_ft, y = y_ft, xend = x_ft+vel_x, yend = y_ft+vel_y), #/sqrt(vel_x^2+vel_y^2) to get r=1
  arrow = arrow(length = unit(0.2, "cm")),size=1, colour='cyan')
```

```{r}
#copy into console
require(gridExtra)
grid.arrange(plot_half_rink(p), plot_half_rink(p2),plot_half_rink(p3),plot_half_rink(p4),
             plot_half_rink(p5),plot_half_rink(p6),plot_half_rink(p7),plot_half_rink(p8),
             plot_half_rink(p9),plot_half_rink(p10),plot_half_rink(p11),ncol=4)
```